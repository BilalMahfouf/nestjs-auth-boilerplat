...
export class CreateCheckoutHandler {
  ...
  async handle(
    ...
    try {
      const processResult = await this.paymentService.process(
        command.amount,
        command.currency,
      );
...
export class CreateCheckoutEndpoint {
  ...
  @ApiOperation({
    ...
    description:
      'Method: POST. Path: /api/v1/payments/checkout. Headers: Authorization: Bearer <access-token>. Body: amount (positive number, max 2 decimals), currency (3-letter ISO code). Side effects: creates a pending payment row in the database and calls the configured payment service abstraction for checkout processing.',
  })
  @ApiAccessTokenAuth()
  ...
  async execute(
    ...
    try {
      return await this.handler.handle(currentUser.userId, command);
    } catch (error: unknown) {
      if (
