...
export class LoginHandler {
    ...
    async handle(command: LoginCommandDto, response: Response): Promise<LoginResponseDto> {
        ...
        const validPassword = await bcrypt.compare(command.password, user.passwordHash);
...
export class LoginEndpoint {
    ...
    @ApiValidationErrorResponse()
    @ApiErrorResponses(UsersErrorDocs.userNotFoundByEmail, UsersErrorDocs.invalidCredentials)
    @ApiUnexpectedServerErrorResponse()
    async execute(
